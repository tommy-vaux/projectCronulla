...
{	
	...
	class MaterialGroup {
        ...
        public float _AlbedoIntensity;
        public float _MAlbedoIntensity;
        public Color _MTint;
        public float _Metallic;
        public Color _MUnderlyingDiffuseTint;
        public float _UnderlyingMetallic;
        public float _Paint1Metallic;
        public float _Paint2Metallic;
        public Color _Paint1MColor;
        public Color _Paint2MColor;
    ...
    static Texture albedoMetallicChart;
    static Texture metallicChart;
    ...
    static bool allowDisablePreviewCameraWhenNotOverSurforgeWindow = false;
	...
	static void SetupWindow() {
        ...
        string surforgePath =  Surforge.GetSurforgePath();
		...
		logoBackgroundTexture = (Texture2D)AssetDatabase.LoadAssetAtPath(surforgePath + "/Editor/Interface/Gui/logoBackground.psd", typeof(Texture2D));
		logoBackgroundTextureLite = (Texture2D)AssetDatabase.LoadAssetAtPath(surforgePath + "/Editor/Interface/Gui/logoBackgroundLite.psd", typeof(Texture2D));

		surforgeLogo = (Texture)AssetDatabase.LoadAssetAtPath(surforgePath + "/Editor/Interface/Gui/surforgeLogo.png", typeof(Texture));
		surforgeLogoLite = (Texture)AssetDatabase.LoadAssetAtPath(surforgePath + "/Editor/Interface/Gui/surforgeLogoLite.png", typeof(Texture));

		startScreenNewTexture = (Texture)AssetDatabase.LoadAssetAtPath(surforgePath + "/Editor/Interface/Gui/newTexture.psd", typeof(Texture));
        startScreenNewTextureMetallic = (Texture)AssetDatabase.LoadAssetAtPath(surforgePath + "/Editor/Interface/Gui/newTextureMetallic.psd", typeof(Texture));
        startScreenUserGuide = (Texture)AssetDatabase.LoadAssetAtPath(surforgePath + "/Editor/Interface/Gui/userGuide.psd", typeof(Texture));
		startScreenVideos = (Texture)AssetDatabase.LoadAssetAtPath(surforgePath + "/Editor/Interface/Gui/videos.psd", typeof(Texture));

		startScreenNewTextureLite = (Texture)AssetDatabase.LoadAssetAtPath(surforgePath + "/Editor/Interface/Gui/newTextureLite.psd", typeof(Texture));
        startScreenNewTextureMetallicLite = (Texture)AssetDatabase.LoadAssetAtPath(surforgePath + "/Editor/Interface/Gui/newTextureMetallicLite.psd", typeof(Texture));
        startScreenUserGuideLite = (Texture)AssetDatabase.LoadAssetAtPath(surforgePath + "/Editor/Interface/Gui/userGuideLite.psd", typeof(Texture));
		startScreenVideosLite = (Texture)AssetDatabase.LoadAssetAtPath(surforgePath + "/Editor/Interface/Gui/videosLite.psd", typeof(Texture));

		menuHover = (Texture)AssetDatabase.LoadAssetAtPath(surforgePath + "/Editor/Interface/Gui/menuHover.psd", typeof(Texture)); 
		menuHoverLite = (Texture)AssetDatabase.LoadAssetAtPath(surforgePath + "/Editor/Interface/Gui/menuHoverLite.psd", typeof(Texture)); 

		glossinessChart = (Texture)AssetDatabase.LoadAssetAtPath(surforgePath + "/Editor/Interface/Gui/glossinessChart.psd", typeof(Texture)); 
		specularChart = (Texture)AssetDatabase.LoadAssetAtPath(surforgePath + "/Editor/Interface/Gui/specularChart.psd", typeof(Texture));
		albedoChart = (Texture)AssetDatabase.LoadAssetAtPath(surforgePath + "/Editor/Interface/Gui/albedoChart.psd", typeof(Texture));
        albedoMetallicChart = (Texture)AssetDatabase.LoadAssetAtPath(surforgePath + "/Editor/Interface/Gui/albedoMetallicChart.psd", typeof(Texture));
        metallicChart = (Texture)AssetDatabase.LoadAssetAtPath(surforgePath + "/Editor/Interface/Gui/metallicChart.psd", typeof(Texture));
	...
	static void SurforgeNewScene(bool isMetallic) {  
	...
	static void LoadGuiIcons() {
        ...
        string iconsPath = surforgePath + "/Editor/Interface/Gui/IconsLite/";
		if (EditorGUIUtility.isProSkin) {
			iconsPath = surforgePath + "/Editor/Interface/Gui/IconsDark/";
		}
	...
	void OnGUI() {
		...
		if (!Application.isPlaying) {
                ...
                if (window != null) {
                    if (Surforge.surforgeSettings.extentTexturePreview != null) {
                        if (Surforge.surforgeSettings.extentTexturePreview.previewCamera != null) {
                            ...
                            //stop texture preview camera rendering when mouse not over Surforge window
                            if ( ((EditorWindow.mouseOverWindow == window) || (isDragStartedOnPreview && (Event.current.type == EventType.MouseDrag))) &&
                                (window.position.size.x > window.minSize.x) ) {

                                Surforge.surforgeSettings.extentTexturePreview.previewCamera.enabled = true;
                            }
                            else {
                                if (allowDisablePreviewCameraWhenNotOverSurforgeWindow) {

                                    if ((Surforge.surforgeSettings.cameraDeactivateTimer - Time.realtimeSinceStartup) < 0) {
                                        Surforge.surforgeSettings.extentTexturePreview.previewCamera.enabled = false;
                                    }
					...
					if (Surforge.surforgeSettings.materialButtons == null) {
						...
						if (Surforge.surforgeSettings.materialButtons.Length == 0) renderMaterialIconsRequired = true;
							...
							if (Surforge.surforgeSettings.materialButtons[0] == null) { //this means we have loaded the saved scene, that contains no saved textures
								renderMaterialIconsRequired = true; 
								activeTool = 5;
                                allowDisablePreviewCameraWhenNotOverSurforgeWindow = false;
                                Surforge.surforgeSettings.extentTexturePreview.previewCamera.enabled = true;
                                Surforge.StartGPURender();
                                Surforge.surforgeSettings.extentTexturePreview.previewCamera.enabled = true;
                                Surforge.surforgeSettings.cameraDeactivateTimer = Time.realtimeSinceStartup + 0.5f;
                            }
						}
					}
                ...
                if (materialEditor != null) {
						...
						if (Surforge.surforgeSettings.extentTexturePreview != null) { 
							if (Surforge.surforgeSettings.extentTexturePreview.composer.gameObject != null) { ///
								if (Surforge.surforgeSettings.extentTexturePreview.previewRenderTexture != null) {
									...
									if (shaderMode == 1) {
                                        if (Surforge.surforgeSettings.isMetallic) {
                                            GUI.Label(chartRect, new GUIContent(albedoMetallicChart, texturePreviewTooltip), texturePreviewGuiLabel);
                                        }
                                        else {
									...
									if (shaderMode == 3) {
                                        if (Surforge.surforgeSettings.isMetallic) {
                                            GUI.Label(chartRect, new GUIContent(metallicChart, texturePreviewTooltip), texturePreviewGuiLabel);
                                        }
                                        else {
					...
					if(GUILayout.Button(new GUIContent(newMaterialIcon, "<b><color="+textHeaderColor+">New material set</color></b>"), buttonStyleMini ) ) {
						if (!isMaterialRenamingNow) {
                            ...
                            string surforgePath = Surforge.GetSurforgePath();
                            Material newMaterial = new Material( (Material)(AssetDatabase.LoadAssetAtPath(surforgePath + "/ComposerPresets/newMaterialPreset.mat", typeof(Material))) );
					...
					if(GUILayout.Button(new GUIContent(saveMaterialPresetIcon, "<b><color="+textHeaderColor+">Save material set </color></b>\n\nSave custom material set to <color="+textHotkeyColor+">Surforge/CustomPresets/MaterialSets/</color> folder\n\nThe existing custom set with the same name will be updated\n\nSaved custom sets will appear next time along with default sets"), buttonStyleMini ) ) {
						if (!isMaterialRenamingNow) { 
                            ...
                            if (!Directory.Exists(surforgePath + "/CustomPresets/MaterialSets/")) {
								Directory.CreateDirectory(surforgePath + "/CustomPresets/MaterialSets/");
							}
                            ...
                            newMaterialPreset.SetFloat("_GuiMode", 0);
							...
							AssetDatabase.CreateAsset(newMaterialPreset, surforgePath + "/CustomPresets/MaterialSets/" + Surforge.surforgeSettings.sceneMaterials[Surforge.surforgeSettings.activeSceneMaterialNumber].name + ".mat");
						}
					}
					...
					if(GUILayout.Button(new GUIContent(saveMaterialPresetIcon, "<b><color="+textHeaderColor+">Save current material</color></b>\n\nSave current material to <color="+textHotkeyColor+">Surforge/CustomPresets/Materials/</color> folder\n\nSaved materials will appear next time along with default materials"), buttonStyleMini ) ) {
						if (!isMaterialRenamingNow) { 
							...
							if (r) {
								...
								AssetDatabase.CreateAsset(newMaterial, surforgePath + "/CustomPresets/Materials/" + System.DateTime.Now.ToString("yyyy-MM-dd_HH-mm-ss") + ".mat");
							}
						}
					}
					...
					if (!isScrollNeedUpdate) {
                        ...
                        GUILayout.BeginArea(new Rect(new Vector2(0, 0), new Vector2(10000, 10000)));
#endif
				...
				if ( (mousePos.x > previewRect.xMin) && (mousePos.y > previewRect.yMin) && (mousePos.x < previewRect.xMax)&& (mousePos.y < previewRect.yMax) ) {
					...
					if ( (Event.current.type == EventType.MouseUp) && (Event.current.button == 1) && (!Event.current.control) && (!Event.current.alt) && (!Event.current.shift) ) { ///
						if (mousePos == rightButtonDownPos) {
                            ...
                            string specMetallText = "specular";
                            if (Surforge.surforgeSettings.isMetallic) {
                                specMetallText = "metallic";
                            }
							...
							menu.AddItem(new GUIContent(specMetallText), shaderMode == 3 ? true : false, SetShaderMode, 3);
			...
			//surforge scene not loaded - show selcome screen
                ...
                if (logoBackgroundTexture == null) {
                    string surforgePath = Surforge.GetSurforgePath();
                    logoBackgroundTexture = (Texture2D)AssetDatabase.LoadAssetAtPath(surforgePath + "/Editor/Interface/Gui/logoBackground.psd", typeof(Texture2D));
                }
                if (logoBackgroundTextureLite == null) {
                    string surforgePath = Surforge.GetSurforgePath();
                    logoBackgroundTextureLite = (Texture2D)AssetDatabase.LoadAssetAtPath(surforgePath + "/Editor/Interface/Gui/logoBackgroundLite.psd", typeof(Texture2D));
                }
				...
				if (EditorGUIUtility.isProSkin) {
                    ...
                    if (GUI.Button(versionPos, version, versionGuiStyle)) {
                        ShowSurforgeAbout();
                    }
				}
				else {
                    GUILayout.Label(surforgeLogoLite, logoGuiStyle);
                    if (GUI.Button(versionPos, version, versionGuiStyleLite)) {
                        ShowSurforgeAbout();
                    }
				}
                ...
                if (EditorGUIUtility.isProSkin) {
                    if (GUILayout.Button(startScreenNewTextureMetallic, startScreenButton, GUILayout.Width(253), GUILayout.Height(57))) {
                        SurforgeNewScene(true);
                    }
                }
                else {
                    if (GUILayout.Button(startScreenNewTextureMetallicLite, startScreenButton, GUILayout.Width(253), GUILayout.Height(57))) {
                        SurforgeNewScene(true);
                    }
                }
	...
	*/
	...
	private static void SurforgeOnScene(SceneView sceneview) {
		...
		//no sufrorge setttings found
			...
			if (Surforge.surforgeSettings) {
				Surforge.VoxelEngineActivate(false);
				StartSurforge();
			}
		}
	...
	static void UpdatePreview() {
        allowDisablePreviewCameraWhenNotOverSurforgeWindow = true;
			...
			if (Surforge.surforgeSettings) {
				...
				if (Surforge.surforgeSettings.isPlaceToolActive) {
                    ...
                    if ( (Event.current.type == EventType.KeyDown) && (Event.current.keyCode == KeyCode.Keypad2) ) {
                        if (!mouseInsideWindow) {
                            Surforge.LogAction("Scale Z decrease", " Numpad 2 ", "");
                            Surforge.surforgeSettings.placeToolPreview.transform.localScale =
                                new Vector3(Surforge.surforgeSettings.placeToolPreview.transform.localScale.x,
                                            Surforge.surforgeSettings.placeToolPreview.transform.localScale.y,
                                            Surforge.surforgeSettings.placeToolPreview.transform.localScale.z - 0.1f);
                        }
                    }
                    if ( (Event.current.type == EventType.KeyDown) && (Event.current.keyCode == KeyCode.Keypad8)) {
                        if (!mouseInsideWindow) {
                            Surforge.LogAction("Scale Z increase", " Numpad 8 ", "");
                            Surforge.surforgeSettings.placeToolPreview.transform.localScale =
                                new Vector3(Surforge.surforgeSettings.placeToolPreview.transform.localScale.x,
                                            Surforge.surforgeSettings.placeToolPreview.transform.localScale.y,
                                            Surforge.surforgeSettings.placeToolPreview.transform.localScale.z + 0.1f);
                    ...
                    if ((Event.current.type == EventType.KeyDown) && (Event.current.keyCode == KeyCode.Keypad4)) {
                        if (!mouseInsideWindow) {
                            Surforge.LogAction("Scale X decrease", " Numpad 4 ", "");
                            Surforge.surforgeSettings.placeToolPreview.transform.localScale =
                                new Vector3(Surforge.surforgeSettings.placeToolPreview.transform.localScale.x - 0.1f,
                                            Surforge.surforgeSettings.placeToolPreview.transform.localScale.y,
                                            Surforge.surforgeSettings.placeToolPreview.transform.localScale.z);
                        }
                    }
                    if ((Event.current.type == EventType.KeyDown) && (Event.current.keyCode == KeyCode.Keypad6)) {
                        if (!mouseInsideWindow) {
                            Surforge.LogAction("Scale X increase", " Numpad 6 ", "");
                            Surforge.surforgeSettings.placeToolPreview.transform.localScale =
                                new Vector3(Surforge.surforgeSettings.placeToolPreview.transform.localScale.x + 0.1f,
                                            Surforge.surforgeSettings.placeToolPreview.transform.localScale.y,
                                            Surforge.surforgeSettings.placeToolPreview.transform.localScale.z);
                    ...
                    if ((Event.current.type == EventType.KeyDown) && (Event.current.keyCode == KeyCode.Keypad2) && Event.current.control) {
                        if (!mouseInsideWindow) {
                            Surforge.LogAction("Scale Z * 0.5", " Ctrl + Numpad 2 ", "");
                            Surforge.surforgeSettings.placeToolPreview.transform.localScale =
                                new Vector3(Surforge.surforgeSettings.placeToolPreview.transform.localScale.x,
                                            Surforge.surforgeSettings.placeToolPreview.transform.localScale.y,
                                            Surforge.surforgeSettings.placeToolPreview.transform.localScale.z * 0.5f);
                        }
                    }
                    if ((Event.current.type == EventType.KeyDown) && (Event.current.keyCode == KeyCode.Keypad8) && Event.current.control) {
                        if (!mouseInsideWindow) {
                            Surforge.LogAction("Scale Z * 2", " Ctrl + Numpad 8 ", "");
                            Surforge.surforgeSettings.placeToolPreview.transform.localScale =
                                new Vector3(Surforge.surforgeSettings.placeToolPreview.transform.localScale.x,
                                            Surforge.surforgeSettings.placeToolPreview.transform.localScale.y,
                                            Surforge.surforgeSettings.placeToolPreview.transform.localScale.z * 2);
                        }
                    }
                    if ((Event.current.type == EventType.KeyDown) && (Event.current.keyCode == KeyCode.Keypad4) && Event.current.control) {
                        if (!mouseInsideWindow) {
                            Surforge.LogAction("Scale X * 0.5", " Ctrl + Numpad 4 ", "");
                            Surforge.surforgeSettings.placeToolPreview.transform.localScale =
                                new Vector3(Surforge.surforgeSettings.placeToolPreview.transform.localScale.x * 0.5f,
                                            Surforge.surforgeSettings.placeToolPreview.transform.localScale.y,
                                            Surforge.surforgeSettings.placeToolPreview.transform.localScale.z);
                        }
                    }
                    if ((Event.current.type == EventType.KeyDown) && (Event.current.keyCode == KeyCode.Keypad6) && Event.current.control) {
                        if (!mouseInsideWindow) {
                            Surforge.LogAction("Scale X * 2", " Ctrl + Numpad 6 ", "");
                            Surforge.surforgeSettings.placeToolPreview.transform.localScale =
                                new Vector3(Surforge.surforgeSettings.placeToolPreview.transform.localScale.x * 2,
                                            Surforge.surforgeSettings.placeToolPreview.transform.localScale.y,
                                            Surforge.surforgeSettings.placeToolPreview.transform.localScale.z);
                        }
                    }
...
// ------------------------------------------------------------------------------
		...
		if ((e.type == EventType.MouseUp)  && e.button == 0) {
			...
			if ( placeToolState == 1 ) {
				...
				if (placeToolIsRotationStarted) {
                    ...
                    if (Surforge.surforgeSettings.placeMeshes.placeMeshes[Surforge.surforgeSettings.activePlaceMesh].autoShuffle) {
                        if (Surforge.surforgeSettings.placeMeshes.placeMeshes[Surforge.surforgeSettings.activePlaceMesh].shuffleArray != null) {
                            if (Surforge.surforgeSettings.placeMeshes.placeMeshes[Surforge.surforgeSettings.activePlaceMesh].shuffleArray.Length > 0) {
                                ShufflePlaceMeshVariants();
                            }
    ...
    static void ShufflePlaceMeshVariants() {
        Surforge.LogAction("Shuffle variants", "Right Click", "");

        Surforge.PlaceToolShuffle();
        Surforge.CreatePlaceToolSymmetryPreviews();

        if (Surforge.surforgeSettings.placeMeshes.placeMeshes[Surforge.surforgeSettings.activePlaceMesh].flipWhileShuffle) {
            int flipRandom = Random.Range(0, 3);
            if (flipRandom == 1) {
                Surforge.surforgeSettings.placeToolPreview.transform.localScale = new Vector3(-Surforge.surforgeSettings.placeToolPreview.transform.localScale.x,
                                                                                Surforge.surforgeSettings.placeToolPreview.transform.localScale.y,
                                                                                Surforge.surforgeSettings.placeToolPreview.transform.localScale.z);
	...
	static GameObject PlaceToolPlaceObject(GameObject placeToolPreview) {
		...
		if (renderer) {
            ...
            bool needSetPlaceObjectTextures = true;
            PlaceMesh placeMesh = Surforge.surforgeSettings.placeMeshes.placeMeshes[Surforge.surforgeSettings.activePlaceMesh];

            childrenHidden = placeMesh.childrenHidden;

            if (placeMesh.shuffleBumpMaps != null) {
                if (placeMesh.shuffleBumpMaps.Length > 0) {
                    needSetPlaceObjectTextures = false;
                }
            }

            if (needSetPlaceObjectTextures) {
	...
	static MaterialGroup GetMaterialGroup(int materialSetNum, int groupNum) {
        ...
        materialGroup._AlbedoIntensity = Surforge.surforgeSettings.sceneMaterials[materialSetNum].GetFloat("_" + groupNum + "AlbedoIntensity");
        materialGroup._MAlbedoIntensity = Surforge.surforgeSettings.sceneMaterials[materialSetNum].GetFloat("_" + groupNum + "MAlbedoIntensity");
        materialGroup._MTint = Surforge.surforgeSettings.sceneMaterials[materialSetNum].GetColor("_MTint" + groupNum);
        materialGroup._Metallic = Surforge.surforgeSettings.sceneMaterials[materialSetNum].GetFloat("_Metallic" + groupNum);
        materialGroup._MUnderlyingDiffuseTint = Surforge.surforgeSettings.sceneMaterials[materialSetNum].GetColor("_" + groupNum + "MUnderlyingDiffuseTint");
        materialGroup._UnderlyingMetallic = Surforge.surforgeSettings.sceneMaterials[materialSetNum].GetFloat("_" + groupNum + "UnderlyingMetallic");
        materialGroup._Paint1Metallic = Surforge.surforgeSettings.sceneMaterials[materialSetNum].GetFloat("_" + groupNum + "Paint1Metallic");
        materialGroup._Paint2Metallic = Surforge.surforgeSettings.sceneMaterials[materialSetNum].GetFloat("_" + groupNum + "Paint2Metallic");
        materialGroup._Paint1MColor = Surforge.surforgeSettings.sceneMaterials[materialSetNum].GetColor("_" + groupNum + "Paint1MColor");
        materialGroup._Paint2MColor = Surforge.surforgeSettings.sceneMaterials[materialSetNum].GetColor("_" + groupNum + "Paint2MColor");
	...
	static void SetMaterialGroup(int groupNum, MaterialGroup materialGroup) {
        ...
        composerMaterial.SetFloat("_" + groupNum + "AlbedoIntensity", materialGroup._AlbedoIntensity);
        composerMaterial.SetFloat("_" + groupNum + "MAlbedoIntensity", materialGroup._MAlbedoIntensity);
        composerMaterial.SetColor("_MTint" + groupNum, materialGroup._MTint);
        composerMaterial.SetFloat("_Metallic" + groupNum, materialGroup._Metallic);
        composerMaterial.SetColor("_" + groupNum + "MUnderlyingDiffuseTint", materialGroup._MUnderlyingDiffuseTint);
        composerMaterial.SetFloat("_" + groupNum + "UnderlyingMetallic", materialGroup._UnderlyingMetallic);
        composerMaterial.SetFloat("_" + groupNum + "Paint1Metallic", materialGroup._Paint1Metallic);
        composerMaterial.SetFloat("_" + groupNum + "Paint2Metallic", materialGroup._Paint2Metallic);
        composerMaterial.SetColor("_" + groupNum + "Paint1MColor", materialGroup._Paint1MColor);
        composerMaterial.SetColor("_" + groupNum + "Paint2MColor", materialGroup._Paint2MColor);
    }
	...
	static Material CreateSurforgeMaterialFromProjectMaterial(Material projectMat) {
        ...
        if (Surforge.surforgeSettings.isMetallic) {
            if (projectMat.shader == Shader.Find("Standard")) {
                Surforge.CopyStandardMaterialToChosenID(projectMat, 0, result, 0, true);
            }
        }
        else {
            if (projectMat.shader == Shader.Find("Standard (Specular setup)")) {
                Surforge.CopyStandardMaterialToChosenID(projectMat, 0, result, 0, false);
            }
        }

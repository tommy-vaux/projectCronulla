...
public class Surforge : MonoBehaviour {  
    ...
    static bool surforgePathAutoFind = true;
    static string surforgePathDefault = "Assets/Surforge";
    static string sfPath;


    public static string GetSurforgePath() {
        if (!surforgePathAutoFind) {
            return surforgePathDefault.Replace("\\", "/");
        }
        RecursivelyFindSurforgePath("Assets");
        return sfPath;
    }


    static void RecursivelyFindSurforgePath(string dir) {
        string[] paths = Directory.GetDirectories(dir);
        foreach (string s in paths) {
            if (s.Contains("Editor")) {
                DirectoryInfo dirInfo = new DirectoryInfo(s + "/");
                FileInfo[] fileInfo = dirInfo.GetFiles("Surforge.cs");
                if (fileInfo.Length > 0) {
                    sfPath = s.Substring(0, s.Length - 7);
                    sfPath = sfPath.Replace("\\", "/");
                    break;
                }
                else {
                    RecursivelyFindSurforgePath(s);
                }
            }
            else {
                RecursivelyFindSurforgePath(s);
    ...
    static void GetSettings(bool isMetallic) {
		...
		if (es == null) {
			...
			CreateSettingsObject(isMetallic);
            ...
            surforgeSettings.surforgePath = GetSurforgePath();
        }
	...
	static void LoadCustomMaterialPresets() {
        string surforgePath = GetSurforgePath();
        if (Directory.Exists(surforgePath + "/CustomPresets/MaterialSets/")) {
			DirectoryInfo dirInfo = new DirectoryInfo(surforgePath + "/CustomPresets/MaterialSets/");
			...
			for (int i=0; i < fileInfo.Length; i++) {
				...
				if (!isPresetNameExists) {
					Material newMaterial = new Material((Material)AssetDatabase.LoadAssetAtPath(surforgePath + "/CustomPresets/MaterialSets/" + fileInfo[i].Name, typeof(Material)) );
	...
	public static void ReloadProjectMaterials() {
		...
		if(Directory.Exists(surforgePath + "/CustomPresets/Materials/")) {
			DirectoryInfo customMaterialsDirectoryInfo = new DirectoryInfo(surforgePath + "/CustomPresets/Materials/");
			FileInfo[] customMaterialsFileInfo = customMaterialsDirectoryInfo.GetFiles("*.mat");
			for (int i=0; i < customMaterialsFileInfo.Length; i++) {
				Material newMaterial = (Material)AssetDatabase.LoadAssetAtPath(surforgePath + "/CustomPresets/Materials/" + customMaterialsFileInfo[i].Name, typeof(Material));
				if (newMaterial != null) {
		...
		DirectoryInfo materialsDirectoryInfo = new DirectoryInfo(surforgePath + "/SurforgeMaterials");
		...
		for (int i=0; i < materialFolders.Length; i++) {
			...
			string[] materialGuids = AssetDatabase.FindAssets("t:Material", new string[] { surforgePath + "/SurforgeMaterials" + "/" + materialFolders[i].Name });
	...
	public static void CreateSettingsObject(bool isMetallic) {
        ...
        surforgeSettings.surforgePath = surforgePath;
        surforgeSettings.isMetallic = isMetallic;
        ...
        surforgeSettings.exportDiffuseMetallic = Shader.Find("Hidden/ExportDiffuseMetallic");
        surforgeSettings.exportMetallic = Shader.Find("Hidden/ExportMetallic");

        surforgeSettings.grayBackgroundPost = Shader.Find("Hidden/GrayBackgroundPost");

		Mesh previewCube = (Mesh)AssetDatabase.LoadAssetAtPath(surforgePath + "/TexturePreview/PreviewMeshes/preview_Cube.fbx", typeof(Mesh));
		...
		surforgeSettings.renderMaterialIconCameraPrefab = (Camera)AssetDatabase.LoadAssetAtPath(surforgePath + "/GPURender/renderMaterialIconCamera.prefab", typeof(Camera));
		surforgeSettings.renderMaterialIconCameraLitePrefab = (Camera)AssetDatabase.LoadAssetAtPath(surforgePath + "/GPURender/renderMaterialIconCameraLite.prefab", typeof(Camera));
		surforgeSettings.renderMaterialIconPrefab = (GameObject)AssetDatabase.LoadAssetAtPath(surforgePath + "/GPURender/renderMaterialIcon.prefab", typeof(GameObject));
		surforgeSettings.renderMaterialIconShader = Shader.Find("Hidden/SurforgeSingle");
		surforgeSettings.rgbaNoise = (Texture2D)AssetDatabase.LoadAssetAtPath(surforgePath + "/Textures/noiseRGBA_01.psd", typeof(Texture2D));
		surforgeSettings.renderMaterialIconNormal = (Texture2D)AssetDatabase.LoadAssetAtPath(surforgePath + "/GPURender/materialIconRender_normal.png", typeof(Texture2D));
		surforgeSettings.renderMaterialIconAoEdgesDirtDepth = (Texture2D)AssetDatabase.LoadAssetAtPath(surforgePath + "/GPURender/materialIconRender_aoEdgesDirtDepth.png", typeof(Texture2D));
		surforgeSettings.renderMaterialIconNoise = (Texture2D)AssetDatabase.LoadAssetAtPath(surforgePath + "/GPURender/materialIconRender_noise.psd", typeof(Texture2D));
		surforgeSettings.editorPreviewAoEdgesDirtDepth = (Texture2D)AssetDatabase.LoadAssetAtPath(surforgePath + "/GPURender/editorPreview_aoEdgesDirtDepth.png", typeof(Texture2D));
		surforgeSettings.editorPreviewMasks = (Texture2D)AssetDatabase.LoadAssetAtPath(surforgePath + "/GPURender/editorPreview_masks.psd", typeof(Texture2D));
		surforgeSettings.editorPreviewMasks2 = (Texture2D)AssetDatabase.LoadAssetAtPath(surforgePath + "/GPURender/editorPreview_masks2.psd", typeof(Texture2D));

		surforgeSettings.uvGridStep = 4;
		surforgeSettings.seamlessHighlight = (Material)AssetDatabase.LoadAssetAtPath(surforgePath + "/GPURender/seamlessHighlight.mat", typeof(Material));
		surforgeSettings.mapsExportCameraPrefab = (GameObject)AssetDatabase.LoadAssetAtPath(surforgePath + "/GPURender/mapsExportCamera.prefab", typeof(GameObject));
		...
		surforgeSettings.polyLassoProfiles = (PolyLassoProfiles)AssetDatabase.LoadAssetAtPath(surforgePath + "/PolyLassoProfiles/polyLassoProfiles.prefab", typeof(PolyLassoProfiles));

		surforgeSettings.decalSets = (DecalSets)AssetDatabase.LoadAssetAtPath(surforgePath + "/Decals/decalSets.prefab", typeof(DecalSets));

		surforgeSettings.noisePresets = (NoisePresets)AssetDatabase.LoadAssetAtPath(surforgePath + "/NoisePresets/noisePresets.prefab", typeof(NoisePresets));

		surforgeSettings.composerPresets = (SurforgeComposerPresets)AssetDatabase.LoadAssetAtPath(surforgePath + "/ComposerPresets/composerPresets.prefab", typeof(SurforgeComposerPresets));

		surforgeSettings.placeMeshes = (PlaceMeshes)AssetDatabase.LoadAssetAtPath(surforgePath + "/AddDetailTool/placeMeshes.prefab", typeof(PlaceMeshes));

		surforgeSettings.shatterPresets = (ShatterPresets)AssetDatabase.LoadAssetAtPath(surforgePath + "/ShatterPresets/shatterPresets.prefab", typeof(ShatterPresets));
	...
	public static void LoadSkyboxes() {
        ...
        string[] skyboxGuids = AssetDatabase.FindAssets("t:Prefab", new string[] { surforgePath + "/Skybox" });
	...
	public static void LoadGreebles() {
        ...
        bool shouldLoadGreebles = false;
		if (surforgeSettings.contentPacks == null) {
            shouldLoadGreebles = true;
        }
        else {
            if (surforgeSettings.contentPacks.Length == 0) {
                shouldLoadGreebles = true;
            }
        }

        if (shouldLoadGreebles) {
            surforgeSettings.contentPacks = new SurforgeContentPack[1];
            surforgeSettings.contentPacks[0] = (SurforgeContentPack)AssetDatabase.LoadAssetAtPath(surforgePath + "/GreeblesTool/greeblesTool.prefab", typeof(SurforgeContentPack));
        }
	}


	public static void VoxelEngineActivate (bool isMetallic) {
		GetSettings(isMetallic);
	...
	*/
	...
	static void InstantiateTexturePreview() {
        ...
        surforgeSettings.extentTexturePreview = (SurforgeTexturePreview)Instantiate(AssetDatabase.LoadAssetAtPath(surforgePath + "/TexturePreview/texturePreview.prefab", typeof(SurforgeTexturePreview)));
        ...
        if (surforgeSettings.isMetallic) {
            renderer.sharedMaterial.shader = Shader.Find("Hidden/ComposerMetallic");
        }
        else {
            renderer.sharedMaterial.shader = Shader.Find("Hidden/Composer");
        }
	...
	public static void CreatePolygonLassoPlane() {
		...
		// Workaround for Unity 2018.1.6f1 mesh collider update issue
		MeshCollider meshCollider = (MeshCollider)surforgeSettings.polygonLassoPlane.GetComponent<MeshCollider>();
	...
	public static void CreatePlaceToolPlane() {
		...
		// Workaround for Unity 2018.1.6f1 mesh collider update issue
		MeshCollider meshCollider = (MeshCollider)surforgeSettings.placeToolPlane.GetComponent<MeshCollider>();
		if (meshCollider != null) {
			meshCollider.enabled = false;
			meshCollider.enabled = true;
		}
			...
			if (surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].shuffleArray != null) {
				if (surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].shuffleArray.Length > 0) {
					...
					if (surforgeSettings.placeToolPreview != null) {
                        ...
                        PlaceMesh chosenPlaceMesh = (PlaceMesh)surforgeSettings.placeToolPreview.GetComponent<PlaceMesh>();
                        if (chosenPlaceMesh.shuffleRotation90) {
                            int rotationSteps = Random.Range(-3, 4);
                            surforgeSettings.placeToolPreview.transform.localEulerAngles = new Vector3(surforgeSettings.placeToolPreview.transform.localEulerAngles.x,
                                                                                                     90.0f * rotationSteps,
                                                                                                     surforgeSettings.placeToolPreview.transform.localEulerAngles.z);
                        }
                        if (chosenPlaceMesh.shuffleRotation180) {
                            int rotationSteps = Random.Range(0, 2);
                            surforgeSettings.placeToolPreview.transform.localEulerAngles = new Vector3(surforgeSettings.placeToolPreview.transform.localEulerAngles.x,
                                                                                                     180.0f * rotationSteps,
                                                                                                     surforgeSettings.placeToolPreview.transform.localEulerAngles.z);
                        }

                        //shuffle AO and Normal maps
                        if ( (surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].shuffleBumpMaps != null) &&
                             (surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].shuffleBumpMapsIntensity != null) &&
                             (surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].shuffleAoMaps != null) &&
                             (surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].aoMapsIntensity != null) ) {

                            if ( (surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].shuffleBumpMaps.Length > 0) &&
                                 (surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].shuffleBumpMapsIntensity.Length > 0) &&
                                 (surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].shuffleAoMaps.Length > 0) &&
                                 (surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].aoMapsIntensity.Length > 0) ) {

                                int mapsShuffleArrayLength = surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].shuffleBumpMaps.Length;
                                if ( (surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].shuffleBumpMapsIntensity.Length == mapsShuffleArrayLength) &&
                                     (surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].shuffleAoMaps.Length == mapsShuffleArrayLength) &&
                                     (surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].aoMapsIntensity.Length == mapsShuffleArrayLength) ) {

                                    int mapsShuffleRandom = Random.Range(0, mapsShuffleArrayLength);
                                    chosenPlaceMesh.bumpMap = surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].shuffleBumpMaps[mapsShuffleRandom];
                                    chosenPlaceMesh.bumpMapIntensity = surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].shuffleBumpMapsIntensity[mapsShuffleRandom];
                                    chosenPlaceMesh.aoMap = surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].shuffleAoMaps[mapsShuffleRandom];
                                    chosenPlaceMesh.aoMapIntensity = surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].aoMapsIntensity[mapsShuffleRandom];

                                    Renderer renderer = chosenPlaceMesh.GetComponent<Renderer>();
                                    ...
                                    float offsetX = 0;
                                    float offsetZ = 0;
                                    if (chosenPlaceMesh.randomUvOffset) {
                                        offsetX = Random.Range(-1.0f, 1.0f);
                                        offsetZ = Random.Range(-1.0f, 1.0f);
                                    }

                                    renderer.sharedMaterial.SetTexture("_BumpMap", chosenPlaceMesh.bumpMap);
                                    renderer.sharedMaterial.SetTextureScale("_BumpMap", chosenPlaceMesh.bumpMapTiling);
                                    renderer.sharedMaterial.SetFloat("_BumpIntensity", chosenPlaceMesh.bumpMapIntensity);

                                    renderer.sharedMaterial.SetTextureOffset("_BumpMap", new Vector2(offsetX, offsetZ));
                                    
                                    renderer.sharedMaterial.SetTexture("_AO", chosenPlaceMesh.aoMap);
                                    renderer.sharedMaterial.SetTextureScale("_AO", chosenPlaceMesh.bumpMapTiling);
                                    renderer.sharedMaterial.SetFloat("_AOIntensity", chosenPlaceMesh.aoMapIntensity);
                                    renderer.sharedMaterial.SetTextureOffset("_AO", new Vector2(offsetX, offsetZ));
                                    

                                    //apply shuffled AO map to placeMesh children
                                    foreach (Transform child in chosenPlaceMesh.transform) {
                                        Renderer childRenderer = (Renderer)child.gameObject.GetComponent<Renderer>();
                                        if (childRenderer != null) {
                                            Material childMat = new Material(childRenderer.sharedMaterial);
                                            childRenderer.sharedMaterial = childMat;

                                            childRenderer.sharedMaterial.SetTexture("_AO", chosenPlaceMesh.aoMap);
                                            
                                            float childAoIntensityModifier = 0;
                                            if (surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].aoMapsIntensityRandomRange != null) {
                                                if (surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].aoMapsIntensityRandomRange.Length > 0) {
                                                    int childAoIntensityModifierRandom = Random.Range(0, surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].aoMapsIntensityRandomRange.Length);
                                                    childAoIntensityModifier = surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].aoMapsIntensityRandomRange[childAoIntensityModifierRandom];
                                                }
                                            }
                                            childRenderer.sharedMaterial.SetFloat("_AOIntensity", chosenPlaceMesh.aoMapIntensity + childAoIntensityModifier);
                                        }
                                    }
	...
	static void CreateBackgroundQuad() {
		...
		if (backgroundQuadMaterial == null) backgroundQuadMaterial = (Material)AssetDatabase.LoadAssetAtPath(surforgePath + "/Editor/Interface/Gui/extentBackground.mat", typeof(Material));
		...
		// Workaround for Unity 2018.1.6f1 mesh collider update issue 
		MeshCollider meshCollider = (MeshCollider)surforgeSettings.backgroundQuad.GetComponent<MeshCollider>();
		if (meshCollider != null) {
			meshCollider.enabled = false;
			meshCollider.enabled = true;
		}
	...
	public static void StartGPURender() {
		...
		gpuRender = (GPURender)Instantiate(AssetDatabase.LoadAssetAtPath(surforgePath + "/GPURender/gpuRender.prefab", typeof(GPURender)));
		...
		if ((surforgeSettings != null) && (path != null) && (path != "") && (surforgeSettings.extentTexturePreview != null)) {
			...
			if (surforgeSettings.normalMap != null) {
				...
				*/
                ...
                if (surforgeSettings.exportDiffuseMetallic == null) {
                    surforgeSettings.exportDiffuseMetallic = Shader.Find("Hidden/ExportDiffuseMetallic");
                }
                if (surforgeSettings.exportMetallic == null) {
                    surforgeSettings.exportMetallic = Shader.Find("Hidden/ExportMetallic");
                }
                
                if (surforgeSettings.isMetallic) {
                    ExportMapGpu(surforgeSettings.exportDiffuseMetallic, export1, surforgeSettings.aoEdgesDirtDepth.width, surforgeSettings.aoEdgesDirtDepth.height, isLinearColorSpace);
                }
                else {
                ...
                if (surforgeSettings.isMetallic) {
                    ExportMapGpu(surforgeSettings.exportMetallic, export1, surforgeSettings.aoEdgesDirtDepth.width, surforgeSettings.aoEdgesDirtDepth.height, isLinearColorSpace);
                }
                else {
                ...
                if (surforgeSettings.isMetallic) {
                    WriteTgaSpecGloss(export1, export2, exportPath + textureName + "_Metallic" + ".tga");
                }
                else {
	...
	static void CreateMaterialForExportedMaps(string path, bool isLinearColorSpace) {
		...
		if (isLinearColorSpace) {
            ...
            string[] guidsSpecular = null;
            if (surforgeSettings.isMetallic) {
                guidsSpecular = AssetDatabase.FindAssets(textureName + "_Metallic t:Texture2D", lookFor);
                if (guidsSpecular.Length > 0) {
                    TextureImporter textureImporterMetallic = (TextureImporter)TextureImporter.GetAtPath(pathForMaterialExport + "/" + textureName + "_Metallic.tga");
                    textureImporterMetallic.linearTexture = true;
                    AssetDatabase.ImportAsset(pathForMaterialExport + "/" + textureName + "_Metallic.tga");
                }
            }
            else {
                guidsSpecular = AssetDatabase.FindAssets(textureName + "_Specular t:Texture2D", lookFor);
                if (guidsSpecular.Length > 0) {
		...
		if (guidsMaterial.Length == 0) {
            ...
            Material matForExportedTextures = null;
            if (surforgeSettings.isMetallic) {
                matForExportedTextures = new Material(Shader.Find("Standard"));
            }
            else {
                matForExportedTextures = new Material(Shader.Find("Standard (Specular setup)"));
            }
            ...
            if (surforgeSettings.isMetallic) {
                exportedTexture = (Texture2D)AssetDatabase.LoadAssetAtPath(pathForMaterialExport + "/" + textureName + "_Metallic.tga", typeof(Texture2D));
                if (exportedTexture != null) {
                    matForExportedTextures.SetTexture("_MetallicGlossMap", exportedTexture);
                }
            }
            else {
            ...
            Material foundMaterial = (Material)AssetDatabase.LoadAssetAtPath(pathForMaterialExport + "/" + textureName + ".mat", typeof(Material));

            if (foundMaterial != null) {
                if (surforgeSettings.isMetallic) {
                    if (foundMaterial.shader != Shader.Find("Standard")) {
                        foundMaterial.shader = Shader.Find("Standard");
                        Texture2D exportedTexture = (Texture2D)AssetDatabase.LoadAssetAtPath(pathForMaterialExport + "/" + textureName + "_Metallic.tga", typeof(Texture2D));
                        if (exportedTexture != null) {
                            foundMaterial.SetTexture("_MetallicGlossMap", exportedTexture);
                        }
                    }
                }
                else {
                    if (foundMaterial.shader != Shader.Find("Standard (Specular setup)")) {
                        foundMaterial.shader = Shader.Find("Standard (Specular setup)");
                        Texture2D exportedTexture = (Texture2D)AssetDatabase.LoadAssetAtPath(pathForMaterialExport + "/" + textureName + "_Specular.tga", typeof(Texture2D));
                        if (exportedTexture != null) {
                            foundMaterial.SetTexture("_SpecGlossMap", exportedTexture);
                        }
                    }
                }
            }
	...
	public static void TakePreviewScreenshotWithMaps(bool isShaderInMapsMode) {
		if (surforgeSettings.normalMap != null) {
			...
			if (!isShaderInMapsMode) {
                ...
                if (surforgeSettings.isMetallic) {
                    ExportMapGpu(surforgeSettings.exportDiffuseMetallic, exportDiffuse, screenMapsRes, screenMapsRes, isLinearColorSpace);
                }
                else {
                ...
                if (surforgeSettings.isMetallic) {
                    ExportMapGpu(surforgeSettings.exportMetallic, exportMapSpecular, screenMapsRes, screenMapsRes, isLinearColorSpace);
                }
                else {
	...
	*/
	...
	static void DebugShape(List<Vector3> points, Color color) {
		...
		Material material = new Material( (Shader)AssetDatabase.LoadAssetAtPath(surforgePath + "/GPURender/Surforge0.shader", typeof(Shader)));
		...
		if (renderer != null) {
			...
			if (mat != null) {
				if (pObj != null) {
                    ...
                    //TODO: keep material settings for not "PlaceMesh" objects while changing ID
                    PlaceMesh placeMeshObj = (PlaceMesh)obj.GetComponent<PlaceMesh>();
					...
					if ((isFloater) && (groupNumber < 8)) {
                        ...
                        if (placeMeshObj != null) {
                            if (placeMeshObj.isProxy) {
                                foreach (Transform child in placeMeshObj.transform) {
                                    AssignMaterialGroup(child.gameObject, groupNumber);
                                }
                            }
                            else {
                                mat = new Material(surforgeSettings.materialGroups[groupNumber]);
	...
	public static void CopyMaterialToChosenID(Material sourceMaterial, int sourceGroupNum, Material destMaterial, int destGroupNum) {
        ...
        destMaterial.SetFloat("_" + destGroupNum + "AlbedoIntensity", sourceMaterial.GetFloat("_" + sourceGroupNum + "AlbedoIntensity"));
        destMaterial.SetFloat("_" + destGroupNum + "MAlbedoIntensity", sourceMaterial.GetFloat("_" + sourceGroupNum + "MAlbedoIntensity"));
        destMaterial.SetColor("_MTint" + destGroupNum, sourceMaterial.GetColor("_MTint" + sourceGroupNum));
        destMaterial.SetFloat("_Metallic" + destGroupNum, sourceMaterial.GetFloat("_Metallic" + sourceGroupNum));
        destMaterial.SetColor("_" + destGroupNum + "MUnderlyingDiffuseTint", sourceMaterial.GetColor("_" + sourceGroupNum + "MUnderlyingDiffuseTint"));
        destMaterial.SetFloat("_" + destGroupNum + "UnderlyingMetallic", sourceMaterial.GetFloat("_" + sourceGroupNum + "UnderlyingMetallic"));
        destMaterial.SetFloat("_" + destGroupNum + "Paint1Metallic", sourceMaterial.GetFloat("_" + sourceGroupNum + "Paint1Metallic"));
        destMaterial.SetFloat("_" + destGroupNum + "Paint2Metallic", sourceMaterial.GetFloat("_" + sourceGroupNum + "Paint2Metallic"));
        destMaterial.SetColor("_" + destGroupNum + "Paint1MColor", sourceMaterial.GetColor("_" + sourceGroupNum + "Paint1MColor"));
        destMaterial.SetColor("_" + destGroupNum + "Paint2MColor", sourceMaterial.GetColor("_" + sourceGroupNum + "Paint2MColor"));

    }


	public static void CopyStandardMaterialToChosenID(Material sourceMaterial, int sourceGroupNum, Material destMaterial, int destGroupNum, bool isMetallic) {
		destMaterial.SetColor("_Tint" + destGroupNum, sourceMaterial.GetColor ("_Color"));

        if (isMetallic) {
            if (sourceMaterial.GetTexture("_MetallicGlossMap") == null) {
                destMaterial.SetFloat("_Metallic" + destGroupNum, sourceMaterial.GetFloat("_Metallic"));
            }
        }
        else {
        ...
        if (isMetallic) {
            destMaterial.SetTexture("_SpecularMap" + destGroupNum, sourceMaterial.GetTexture("_MetallicGlossMap"));

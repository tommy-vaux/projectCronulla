...
public class GPURender : MonoBehaviour {
    ...
    void Start() {
        ...
        SetHiddenChildrenVisibility(true);
        ...
        SetHiddenChildrenVisibility(false);
    ...
    void SetHiddenChildrenVisibility(bool isVisible) {
        foreach (Transform child in surforgeSettings.root.transform) {
            PlaceMesh placeMesh = (PlaceMesh)child.gameObject.GetComponent<PlaceMesh>();
            if (placeMesh != null) {
                if (placeMesh.childrenHidden) {

                    foreach (Transform hiddenChild in child) {
                        Renderer hiddenChildRenderer = (Renderer)hiddenChild.gameObject.GetComponent<Renderer>();
                        if (hiddenChildRenderer != null) {
                            hiddenChildRenderer.enabled = isVisible;
                        }
                    }

                    if (surforgeSettings.seamless) {
                        if (placeMesh.seamlessInstances != null) {
                            if (placeMesh.seamlessInstances.Length == 8) {
                                for (int i = 0; i < 8; i++) {
                                    if (placeMesh.seamlessInstances[i]) {
                                        foreach (Transform hiddenInstanceChild in placeMesh.seamlessInstances[i].transform) {
                                            Renderer hiddenInstanceChildRenderer = (Renderer)hiddenInstanceChild.gameObject.GetComponent<Renderer>();
                                            if (hiddenInstanceChildRenderer != null) {
                                                hiddenInstanceChildRenderer.enabled = isVisible;
                                            }
                                        }
                                    }
                                }
